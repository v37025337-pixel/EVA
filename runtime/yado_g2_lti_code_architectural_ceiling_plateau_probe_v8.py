...
OUT=ROOT/'yado_g2_lti_code_architectural_ceiling_plateau_probe_v8_receipt.json'
...
if ledger.get('open_deficits')!=['LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V8']:raise RuntimeError('UNEXPECTED_FRONTIER')
...
for c in range(n+1):
    shift=(c*19+11)%n
    trues={(shift+j*11)%n for j in range(c)}
    x={f'n{i:02d}':i in trues for i in range(n)}
    rows.append({'input':x,'expected':'A' if c%10 in {1,6,9} else 'B'})
...
poly=[{'x':x,'y':y,'expected':3*x**4+2*x**2*y**2+y**4+2*x-3*y+13} for x,y in pts]
...
probes['LOGIC']={'WIDTH34_FRESH':{'score':1.0 if lb else 0.0},'DEGREE4_SHIFT_FRESH_V8':{'score':1.0 if lp else 0.0}}
...
st=[S(f'w{i}',1,.07,1,True,.1,False,()) for i in range(14)]
p=P.plan(.02,1.0,14.0,st)
wscore=1.0 if p.expected_confidence>=.999 and len(p.sequence)==14 else 0.0
ids=[f'd{i}' for i in range(15)]
dep=[S(ids[0],1,.02,1,True,.1,False,())]+[S(ids[j],1,.07,1,True,.1,False,(ids[j-1],)) for j in range(1,len(ids))]
q=P.next_after_observation(.0,1.0,15.0,dep,ids[0],.02)
dscore=1.0 if q.expected_confidence>=.999 and len(q.sequence)>=14 else 0.0
probes['THINKING']={'WIDTH14_FRESH':{'score':wscore},'DEPENDENCY14_FRESH':{'score':dscore}}
...
train=[];test=[];fields=[f'k{i:02d}' for i in range(33)]+['xx_signal']
for k in range(600):
    x={f:bool(((k+59)>>(j%8))&1) for j,f in enumerate(fields)};x['xx_signal']=bool((k//8)%2)
    train.append({'input':x,'expected':(CAP_REL,) if x['xx_signal'] else (CAP_CONJ,)})
for k in range(300):
    x={f:bool(((k+197)>>(j%7))&1) for j,f in enumerate(fields)};x['xx_signal']=bool((k//10)%2)
    test.append({'input':x,'expected':(CAP_REL,) if x['xx_signal'] else (CAP_CONJ,)})
...
probes['INTELLIGENCE']={'WIDTH34_INTELLIGENCE_FRESH':{'score':iw},'PAIRWISE_FRESH_V8':{'score':ip}}
...
for i,p in enumerate(sorted(plane_scores)):
    tok='opaque_'+h({'plateau_probe':8,'slot':i,'head':head['canonical_head_digest']})[:18]
...
if plateau_round:
    ...
    next_cap='LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V9' if streak<req else 'LTI_CODE_ARCHITECTURAL_CEILING_EMPIRICAL_PLATEAU_CONFIRMATION_V1'
...
state['plateau_probe']={'probe_version':8,'probes':probes,'plane_scores':plane_scores,'best_candidate_gain_upper_bound':best_gain,
...
receipt={'schema':'yado.g2.lti_code_architectural_ceiling_plateau_probe.v8',
 'status':'PASS_LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V8' if passed else 'WITHHOLD_LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V8',
 ...
 'semantic_boundary':'THIRD FORMAL PLATEAU ROUND AFTER CODE V11 AND THINKING V2. GAIN IS UPPER-BOUNDED BY REMAINING SCORE HEADROOM ON FRESH FAMILIES; ABSOLUTE CEILING IS NOT CLAIMED.'}
receipt['receipt_sha256']=h(receipt);OUT.write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')

e={'index':len(ledger['events']),'event_id':f"E{len(ledger['events'])+1:04d}_G2_LTI_CODE_PLATEAU_PROBE_V8",
 ...
 'deficit':'LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V8',
 'effect':f"STATUS={probe_status}; STREAK={streak}; SCORES={canon(plane_scores)}; NEXT={next_cap}",
 'source_path':f'receipts/yado-g2-lti-code-architectural-ceiling-plateau-probe-v8-run-{run_id}.json','source_digest':receipt['receipt_sha256'],
...
if not passed:raise SystemExit('LTI_CODE_PLATEAU_PROBE_V8_WITHHELD')
