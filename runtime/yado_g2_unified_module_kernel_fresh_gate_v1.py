...
print(json.dumps({'status':status,'active_module_count':len(active),'covered_module_count':len(coverage),'module_coverage_pass':module_coverage_pass,'interaction_pass':interaction_pass,'functional_assembly_pass':functional_assembly_pass,'canonical_ready':canonical_ready,'binding_checks':binding_checks,'failed_smoke':[k for k,v in smoke.items() if not v.get('pass')],'failed_smoke_details':{k:v for k,v in smoke.items() if not v.get('pass')},'failed_interactions':[k for k,v in interaction.items() if not v.get('pass')],'failed_interaction_details':{k:v for k,v in interaction.items() if not v.get('pass')}},indent=2,sort_keys=True,default=str))
