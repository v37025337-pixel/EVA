...
class CanonicalScaleConditionalRuntimeV1:
    """
    Binding-only runtime.
    The routing program is native COMMITTED DevelopmentalExecutiveV22 state.
    The low/high branch models are already admitted candidate artifacts.
    No threshold, target mapping, or learner is authored here.
    """
    def __init__(self,binding:dict|None=None,repo_root:Path|None=None):
        self.repo=Path(repo_root or REPO)
        self.binding=dict(binding or _load(self.repo/'canonical/yado-native-selector-canonical-binding-v1.json'))
        ...
        tmp=tempfile.NamedTemporaryFile(prefix='yado-selector-registry-',suffix='.sqlite',delete=False)
        tmp.close();self._tmp_db=Path(tmp.name);shutil.copy2(registry,self._tmp_db)
        ...
        self.high=_load(self.repo/self.binding['branch_artifacts']['high_pair_knn'])
    ...
    def _high_branch(self,key,x):
        model=self.high['selected_model']
        return knn_predict(model,self._augment_pair(key,x))

    def predict(self,case:dict):
        key=case['key'];x=case['x']
        route=self.route(x)
        low=self.binding['branches']['low']
        high=self.binding['branches']['high']
        if route==low:return self._old_branch(x)
        if route==high:return self._high_branch(key,x)
        raise RuntimeError('UNKNOWN_SELECTOR_ROUTE:'+str(route))

    def snapshot(self):
        return {
          'schema':'yado.g2.canonical_scale_conditional_runtime.v1',
          ...
          'low_branch':self.binding['branches']['low'],
          'high_branch':self.binding['branches']['high'],
