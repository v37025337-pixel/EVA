...
V15=REPO/'candidates/kernel-self-generated/g2-native-contextual-ast-context-composition-repair-v15.json'
...
OUT=REPO/'candidates/kernel-self-generated/g2-native-contextual-ast-context-composition-genesis-v16.json'
GENE=REPO/'candidates/kernel-self-generated/g2-native-contextual-ast-context-composition-gene-v16.json'
DB=ROOT/'yado_native_contextual_ast_context_composition_genesis_v16.sqlite'

def canon(o):return json.dumps(o,sort_keys=True,separators=(',',':'),default=str)
def digest(o):return hashlib.sha256(canon(o).encode()).hexdigest()
def load(p):return json.loads(Path(p).read_text(encoding='utf-8'))

v15,v12gene,binder,action=map(load,[V15,V12GENE,BINDER,ACTION])
if v15.get('status')!='WITHHOLD_G2_NATIVE_CONTEXTUAL_AST_CONTEXT_COMPOSITION_REPAIR_V15':
    raise RuntimeError('V15_WITHHOLD_REQUIRED')
if v15.get('next_required_capability')!='NATIVE_CONTEXTUAL_AST_CONTEXT_COMPOSITION_GENESIS_V16':
    raise RuntimeError('V15_FRONTIER_MISMATCH')
...
for n in tree.body:
    ...
    loader=call.args[0];seq=call.args[1]
    ...
    row={'pattern':'UNPACK_MAP','loader_name':loader.id,'target_names':target_names,'source_names':source_names,'source':ast.unparse(n)}
    unpack_rows.append(row)
    for dst,src_name in zip(target_names,source_names):
        unpack_bindings[dst]={
          'kind':'UNPACK_MAP_ARTIFACT_BINDING','loader_name':loader.id,'source_symbol':src_name,
          'source_symbol_binding':simple_assign.get(src_name),'source_statement':row['source']
        }

required=set(str(x) for x in (binder.get('contract_fields') or []))
feature_candidates=[]
for n in tree.body:
    if not isinstance(n,ast.Assign) or len(n.targets)!=1 or not isinstance(n.targets[0],ast.Name) or not isinstance(n.value,ast.Dict):continue
...
if len(feature_candidates)!=1:raise RuntimeError('EXPECTED_ONE_EXACT_FEATURE_DICT')
...
for mode in ('SIMPLE_ONLY','SIMPLE_PLUS_UNPACK'):
    closure,unresolved=resolve(mode)
    score=1.0 if set(feature['keys'])==required and not unresolved else 0.0
    strategies.append({
      'mode':mode,'feature_assignment':feature['assignment_name'],'feature_rhs_dump':feature['rhs_dump'],
      'closure':closure,'unresolved':unresolved,'score':score,
      'artifact_binding_count':sum(v.get('kind')=='UNPACK_MAP_ARTIFACT_BINDING' for v in closure.values()),
      'runtime_intrinsic_count':sum(v.get('kind')=='RUNTIME_INTRINSIC' for v in closure.values()),
    })
...
for row in strategies:
    sid='CONTEXT_COMPOSITION_V16_'+row['mode']
    skills.append(SkillCandidate(
      skill_id=sid,artifact_digest=digest(row),
      structural_valid=(row['score']==1.0),semantic_consistency=row['score'],
      ...
      metadata={'origin':'YADO_SOURCE_CONTEXT_COMPOSITION_WITH_RUNTIME_INTRINSICS','mode':row['mode']}
    ))
    row['skill_id']=sid
...
try:
    goal=k.executive.create_goal(
      objective='Complete YADO contextual AST context composition by treating Python runtime intrinsics as already bound while preserving all YADO-derived semantic bindings.',
      required_capabilities={'NATIVE_CONTEXTUAL_AST_CONTEXT_COMPOSITION_GENESIS_V16':1.0},
      success_criteria={'no_unresolved_context':True,'artifact_bindings_source_derived':True,'runtime_intrinsics_bound':True,'rollback':True},
    )
...
closure=(selected or {}).get('closure') or {}
action_binding=closure.get('action') or {}
...
checks={
 ...
 'v15_failure_consumed':True,
 'v15_found_action_and_binder_artifact_bindings':bool((v15.get('checks') or {}).get('action_artifact_binding_found')) and bool((v15.get('checks') or {}).get('binder_artifact_binding_found')),
 'v15_only_remaining_unresolved_was_file_intrinsic':any(x.get('mode')=='SIMPLE_PLUS_UNPACK' and x.get('unresolved')==['__file__'] for x in (v15.get('composition_strategies') or [])),
 'runtime_intrinsic_file_treated_as_bound':all('__file__' not in x.get('unresolved',[]) for x in strategies if x.get('mode')=='SIMPLE_PLUS_UNPACK'),
 ...
 'selected_composition_has_no_unresolved_context':bool(selected and not selected['unresolved']),
 'selected_action_binding_is_yado_artifact_binding':action_binding.get('kind')=='UNPACK_MAP_ARTIFACT_BINDING' and action_binding.get('source_symbol')=='ACTION',
 'selected_binder_binding_is_yado_artifact_binding':binder_binding.get('kind')=='UNPACK_MAP_ARTIFACT_BINDING' and binder_binding.get('source_symbol')=='BINDER',
 ...
 'host_authored_feature_logic':False,'host_authored_artifact_values':False,'host_selected_composition':False,
 'external_models_used':False,'actual_target_source_integration_proven':False,
 'canonical_unchanged':core.head.get('canonical_head_digest')==head_before.get('canonical_head_digest'),
 'automatic_canonical_promotion':False,
}
positive=(
 'canonical_v5_continuity_active','v15_failure_consumed','v15_found_action_and_binder_artifact_bindings',
 'v15_only_remaining_unresolved_was_file_intrinsic','runtime_intrinsic_file_treated_as_bound',
 'exact_feature_dict_preserved_unchanged','native_goal_created','native_deficit_detected',
 'native_skill_selector_executed','native_selector_selected_one_composition',
 'selected_composition_is_simple_plus_unpack','selected_composition_has_no_unresolved_context',
 'selected_action_binding_is_yado_artifact_binding','selected_binder_binding_is_yado_artifact_binding',
 'current_binder_ground_accepts','single_fault_counterfactuals_withhold','threshold_not_lowered','canonical_unchanged'
)
negative=('host_authored_feature_logic','host_authored_artifact_values','host_selected_composition','external_models_used','actual_target_source_integration_proven','automatic_canonical_promotion')
passed=all(checks[k] is True for k in positive) and all(checks[k] is False for k in negative)

gene=None
if passed:
    gene={
      'schema':'yado.g2.native_contextual_ast_context_composition_gene.v16',
      'gene_id':'GENE-G2-NATIVE-AST-CONTEXT-COMPOSITION-V16-'+digest({
        'feature':feature['rhs_dump'],'selected_mode':selected['mode'],'closure':closure,
        'v12':v12gene.get('gene_digest'),'binder':binder.get('gene_digest')
      })[:16],
      'novel_gene':True,
      'gene_scope':['CODE','SELF_AUDIT_AND_REPAIR','GENERATIVE_EXECUTIVE'],
      'origin':'YADO_NATIVE_SELECTION_OVER_SOURCE_DERIVED_CONTEXT_WITH_RUNTIME_INTRINSICS',
      'composition_mode':selected['mode'],'feature_binding':feature,'context_closure':closure,
      'artifact_bindings':{'action':action_binding,'binder':binder_binding},
      'runtime_intrinsics':sorted(k for k,v in closure.items() if v.get('kind')=='RUNTIME_INTRINSIC'),
      'v12_operand_materialization_gene_id':v12gene.get('gene_id'),'binder_gene_id':binder.get('gene_id'),
      'ground_verdict':ground_verdict,'counterfactual_fail_closed':fail_closed,
      'all_context_dependencies_bound':True,'actual_target_source_integration_proven':False,
...
status='PASS_SHADOW_G2_NATIVE_CONTEXTUAL_AST_CONTEXT_COMPOSITION_GENESIS_V16' if passed else 'WITHHOLD_G2_NATIVE_CONTEXTUAL_AST_CONTEXT_COMPOSITION_GENESIS_V16'
next_cap='NATIVE_CONTEXT_BOUND_AST_SOURCE_REALIZATION_V17' if passed else 'NATIVE_CONTEXTUAL_AST_CONTEXT_RUNTIME_BINDING_REPAIR_V17'
report={
 'schema':'yado.g2.native_contextual_ast_context_composition_genesis.v16','status':status,
 'parent_v15_receipt':v15.get('receipt_sha256'),'composition_strategies':strategies,
 ...
 'semantic_boundary':'V16 REPAIRS ONLY THE FINAL CONTEXT-CLOSURE ACCOUNTING DEFECT FROM V15 BY RECOGNIZING STANDARD PYTHON RUNTIME INTRINSICS SUCH AS __file__ AS ALREADY BOUND. ALL FEATURE SEMANTICS, ARTIFACT PATHS, DESTRUCTURING BINDINGS AND NATIVE SKILL THRESHOLDS REMAIN UNCHANGED. PASS CREATES A SHADOW CONTEXT-COMPOSITION GENE; TARGET SOURCE REALIZATION REMAINS FOR V17.'
}
report['receipt_sha256']=digest(report)
OUT.parent.mkdir(parents=True,exist_ok=True)
OUT.write_text(json.dumps(report,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8')
print(json.dumps({
 'status':status,'selected_composition':selected,'gene_id':gene.get('gene_id') if gene else None,
