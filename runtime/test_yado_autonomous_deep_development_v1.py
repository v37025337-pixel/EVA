...
def test_selects_verification_when_regression_is_dirty(tmp_path):
    for relative in (
        "runtime/core.py",
        "successor/agent.py",
    ...
    assert plan["status"] == "PASS_SHADOW_AUTONOMOUS_DEEP_DEVELOPMENT_PLAN_V1"
    assert plan["selected_target"] == "VERIFICATION"
    assert plan["boundaries"]["automatic_main_mutation"] is False
    assert len(plan["development_sequence"]) == 7


def test_withholds_when_a_layer_has_no_evidence(tmp_path):
    _touch(tmp_path, "runtime/core.py")
    plan = build_plan(tmp_path)
    assert plan["status"] == "WITHHOLD_AUTONOMOUS_DEEP_DEVELOPMENT_LAYER_COVERAGE_V1"
    assert plan["selected_target"] in {"INTEGRITY", "VERIFICATION", "LAYERS", "MODULES", "MEMORY_EXPERIENCE"}
