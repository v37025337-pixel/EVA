...
class ExternalDevResidualContinuationV2Tests(unittest.TestCase):
    ...
    def test_restart_after_base_five_continues_two_residual_goals(self):
        ...
        self.assertEqual(final['goal_count'], 7, final)
        self.assertEqual(final['base_goal_count'], 5, final)
        self.assertEqual(final['residual_goal_count'], 2, final)
        self.assertEqual(final['events'][5]['selected_capability'], 'LOCAL_APP_BUILD_ADMISSION_LOOP', final)
        self.assertEqual(final['events'][6]['selected_capability'], 'PURE_LOCAL_DEV_UTILITY_LIBRARY', final)
        self.assertEqual(final['events'][6]['result_status'], 'PASS_RESIDUAL_CONTINUATION_DIGEST', final)
        self.assertEqual(len(final['continuation_digest']), 64, final)
        self.assertIsNone(final['next_goal'], final)
        self.assertEqual(final['status'], 'PASS_SHADOW_EXTERNAL_DEV_RESIDUAL_CONTINUATION_V2', final)
        self.assertEqual(final['restart_count'], 1, final)
        self.assertEqual(final['chain_verification']['event_count'], 7, final)
        self.assertTrue(final['post_fixed_goal_derived_from_live_state'], final)
