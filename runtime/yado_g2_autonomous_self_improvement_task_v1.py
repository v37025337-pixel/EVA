...
def main(argv=None):
    ...
    goal_intake=YADOGoalActionBindingV1.resolve_goal(task,audit)
    priority=goal_intake['priority']
    next_step=priority[0].get('code') if priority else None
    if not next_step: raise RuntimeError(goal_intake['status'])
    ...
    report={
     ...
     'host_created_goal_intake_transport':True,
     ...
     'canonical_head_unchanged':load(REPO/'canonical/yado-main-head-g2.json')==head_before,
