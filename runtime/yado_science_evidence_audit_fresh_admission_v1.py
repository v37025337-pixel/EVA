...
OUT=ROOT/'yado_science_evidence_audit_fresh_admission_v1_receipt.json'
...
if ledger.get('open_deficits')!=['SCIENCE_EVIDENCE_AUDIT_FRESH_ADMISSION_V1']:raise RuntimeError('UNEXPECTED_FRONTIER')
...
source_checks={
 ...
 'science_fresh_receipt_check':"science_fresh_receipt" in source,
 'fresh_dataset_guard':"{'PENGUINS','TIPS'}.issubset(science_fresh_datasets)" in source,
 'stale_native_science_gate_removed':"science_proven=(native_pass.get('REAL_SCIENCE_DATA_TRANSFER')" not in source,
}
...
tmp=ROOT/'_audit_v5_fresh_candidate.py'
tmp.write_text(source,encoding='utf-8')
try:
    sp=importlib.util.spec_from_file_location('_audit_v5_fresh',tmp)
...
next_cap='SCIENCE_EVIDENCE_AUDIT_CANONICAL_INTEGRATION_V1' if passed else 'SCIENCE_EVIDENCE_AUDIT_SELF_EVOLUTION_V2'
run_id=str(os.getenv('GITHUB_RUN_ID') or 'LOCAL')
receipt={'schema':'yado.g2.science_evidence_audit_fresh_admission.v1',
 'status':'PASS_SCIENCE_EVIDENCE_AUDIT_FRESH_ADMISSION_V1' if passed else 'WITHHOLD_SCIENCE_EVIDENCE_AUDIT_FRESH_ADMISSION_V1',
...
ledger=load(LEDGER)
e={'index':len(ledger['events']),'event_id':f"E{len(ledger['events'])+1:04d}_G2_SCIENCE_EVIDENCE_AUDIT_FRESH_ADMISSION",
 'event_type':'SELF_AUDIT_SCIENCE_EVIDENCE_FRESH_ADMISSION','status':'PASS_SHADOW' if passed else 'WITHHOLD',
 'generation':ledger['current_head'],'deficit':'SCIENCE_EVIDENCE_AUDIT_FRESH_ADMISSION_V1',
 'effect':f"AUDIT_V5_FRESH_ADMISSION; NEXT={next_cap}",
 'source_path':f'receipts/yado-science-evidence-audit-fresh-admission-v1-run-{run_id}.json',
...
if not passed:raise SystemExit('SCIENCE_EVIDENCE_AUDIT_FRESH_ADMISSION_WITHHELD')
