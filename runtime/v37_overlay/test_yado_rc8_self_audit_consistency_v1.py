...
class TestRC8SelfAuditConsistency(unittest.TestCase):
    ...
    def test_digital_consciousness_evidence_split_brain_is_detected(self):
        s=copy.deepcopy(self.state)
        s['cognitive_capability_lineage']['digital_consciousness']['external_exact_runtime_verification_pending']=True
        self.assertIn('DIGITAL_CONSCIOUSNESS_EXTERNAL_VERIFICATION_STILL_PENDING',validate_state_self_model(s))
    def test_digital_consciousness_receipt_tamper_is_detected(self):
        s=copy.deepcopy(self.state)
        s['external_runtime']['latest_verified_digital_consciousness']['github_run_id']=1
        self.assertIn('DIGITAL_CONSCIOUSNESS_EXTERNAL_LINEAGE_SPLIT_BRAIN',validate_state_self_model(s))
