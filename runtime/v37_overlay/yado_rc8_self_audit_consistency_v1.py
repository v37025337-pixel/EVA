...
def validate_state_self_model(state:dict[str,Any])->list[str]:
    ...
    dc=lineage.get('digital_consciousness') or {}
    dc_latest=ext.get('latest_verified_digital_consciousness') or {}
    if dc.get('external_exact_runtime_verification_pending') is not False:
        errs.append('DIGITAL_CONSCIOUSNESS_EXTERNAL_VERIFICATION_STILL_PENDING')
    if not dc.get('external_verified'):
        errs.append('DIGITAL_CONSCIOUSNESS_EXTERNAL_PROOF_MISSING')
    if int(dc.get('verified_external_run_id') or 0) <= 0:
        errs.append('DIGITAL_CONSCIOUSNESS_EXTERNAL_RUN_ID_MISSING')
    if not dc.get('receipt_blob_sha') or not dc.get('theory_receipt_blob_sha'):
        errs.append('DIGITAL_CONSCIOUSNESS_RECEIPT_EVIDENCE_MISSING')
    if int(dc_latest.get('github_run_id') or 0) != int(dc.get('verified_external_run_id') or 0):
        errs.append('DIGITAL_CONSCIOUSNESS_EXTERNAL_LINEAGE_SPLIT_BRAIN')
    if not dc_latest.get('independent_readback'):
        errs.append('DIGITAL_CONSCIOUSNESS_INDEPENDENT_READBACK_MISSING')
...
def validate_package_coherence(manifest:dict[str,Any], head:dict[str,Any], state:dict[str,Any])->list[str]:
    ...
    if cb.get('functional_digital_consciousness_external_verification_pending') is not False:
        errs.append('MANIFEST_DIGITAL_CONSCIOUSNESS_PENDING_STALE')
    if cb.get('functional_digital_consciousness_external_verified') is not True:
        errs.append('MANIFEST_DIGITAL_CONSCIOUSNESS_PROOF_MISSING')
    if hb.get('functional_digital_consciousness_external_verification_pending') is not False:
        errs.append('HEAD_DIGITAL_CONSCIOUSNESS_PENDING_STALE')
    if hb.get('functional_digital_consciousness_external_verified') is not True:
        errs.append('HEAD_DIGITAL_CONSCIOUSNESS_PROOF_MISSING')
