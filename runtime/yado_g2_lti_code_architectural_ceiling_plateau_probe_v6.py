...
OUT=ROOT/'yado_g2_lti_code_architectural_ceiling_plateau_probe_v6_receipt.json'
...
if ledger.get('open_deficits')!=['LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V6']:raise RuntimeError('UNEXPECTED_FRONTIER')
...
for c in range(n+1):
    shift=(c*13+5)%n
    trues={(shift+j*7)%n for j in range(c)}
    x={f'l{i:02d}':i in trues for i in range(n)}
    rows.append({'input':x,'expected':'A' if c%8 in {1,4,7} else 'B'})
...
poly=[{'x':x,'y':y,'expected':x**4+2*x**3*y-2*x*y**3+y**4+3*x-4*y+9} for x,y in pts]
...
probes['LOGIC']={'WIDTH30_FRESH':{'score':1.0 if lb else 0.0},'DEGREE4_SHIFT_FRESH':{'score':1.0 if lp else 0.0}}
...
st=[S(f'w{i}',1,.08,1,True,.1,False,()) for i in range(12)]
p=P.plan(.04,1.0,12.0,st)
wscore=1.0 if p.expected_confidence>=.999 and len(p.sequence)==12 else 0.0
ids=[f'd{i}' for i in range(13)]
dep=[S(ids[0],1,.04,1,True,.1,False,())]+[S(ids[j],1,.08,1,True,.1,False,(ids[j-1],)) for j in range(1,len(ids))]
q=P.next_after_observation(.0,1.0,13.0,dep,ids[0],.04)
dscore=1.0 if q.expected_confidence>=.999 and len(q.sequence)>=12 else 0.0
probes['THINKING']={'WIDTH12_FRESH':{'score':wscore},'DEPENDENCY12_FRESH':{'score':dscore}}
...
train=[];test=[];fields=[f'i{i:02d}' for i in range(29)]+['zz_signal']
for k in range(600):
    x={f:bool(((k+37)>>(j%8))&1) for j,f in enumerate(fields)};x['zz_signal']=bool((k//6)%2)
    train.append({'input':x,'expected':(CAP_REL,) if x['zz_signal'] else (CAP_CONJ,)})
for k in range(300):
    x={f:bool(((k+151)>>(j%7))&1) for j,f in enumerate(fields)};x['zz_signal']=bool((k//8)%2)
...
probes['INTELLIGENCE']={'WIDTH30_FRESH':{'score':iw},'PAIRWISE_FRESH_V6':{'score':ip}}
...
amb=[((-9,),18),((-5,),10),((0,),0),((1,),1),((5,),21),((9,),37)]
...
for i,p in enumerate(sorted(plane_scores)):
    tok='opaque_'+h({'plateau_probe':6,'slot':i,'head':head['canonical_head_digest']})[:18]
...
if plateau_round:
    ...
    next_cap='LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V7' if streak<req else 'LTI_CODE_ARCHITECTURAL_CEILING_EMPIRICAL_PLATEAU_CONFIRMATION_V1'
...
state['plateau_probe']={'probe_version':6,'probes':probes,'plane_scores':plane_scores,'best_candidate_gain_upper_bound':best_gain,
...
receipt={'schema':'yado.g2.lti_code_architectural_ceiling_plateau_probe.v6',
 'status':'PASS_LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V6' if passed else 'WITHHOLD_LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V6',
 ...
 'semantic_boundary':'FIRST FORMAL PLATEAU ROUND AFTER CODE V11 AND THINKING V2. GAIN IS UPPER-BOUNDED BY REMAINING SCORE HEADROOM ON FRESH FAMILIES; ABSOLUTE CEILING IS NOT CLAIMED.'}
receipt['receipt_sha256']=h(receipt);OUT.write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')

e={'index':len(ledger['events']),'event_id':f"E{len(ledger['events'])+1:04d}_G2_LTI_CODE_PLATEAU_PROBE_V6",
 ...
 'deficit':'LTI_CODE_ARCHITECTURAL_CEILING_PLATEAU_PROBE_V6',
...
if not passed:raise SystemExit('LTI_CODE_PLATEAU_PROBE_V6_WITHHELD')
