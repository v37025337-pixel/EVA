...
if len(rows)<10:
    raise SystemExit('INSUFFICIENT_EXTERNAL_CORPUS')
...
def vector(ids):
    ...
    # Evidence-dominant family is computed from the external corpus, not supplied by a fixed architecture answer.
...
candidate_supported=(
    result.get('fresh_blind',0)>=0.90
    and result.get('validation',0)>=0.90
    and len(blind_labels)>=4
)
receipt={
 ...
 'kernel_algorithm_bank':bank.get('INTELLIGENCE',[]),
 'kernel_result':result,
 'case_counts':{'all':len(cases),'fit':len(fit),'validation':len(validation),'revealed_refit':len(revealed),'blind':len(blind)},
 'blind_family_count':len(blind_labels),
 'blind_families':blind_labels,
 ...
 'source_digests':[{k:v for k,v in row.items() if k!='text'} for row in rows.values()],
 'admission_requirements':['validation>=0.90','fresh_blind>=0.90','blind_contains>=4_architecture_families'],
 'next_required_capability':(
    'SELF_GENERATED_EXECUTABLE_ARCHITECTURE_FAMILY_CONSTRUCTOR_V1'
    if candidate_supported else
    'IMPROVE_ARCHITECTURE_NEUTRAL_SELECTOR_BEFORE_CONSTRUCTOR'
 ),
 'semantic_boundary':'META_SELECTOR_CAPABILITY_NOT_GENERAL_ARCHITECTURE_SELF_REWRITE_AND_NOT_PROOF_OF_AGI',
}
