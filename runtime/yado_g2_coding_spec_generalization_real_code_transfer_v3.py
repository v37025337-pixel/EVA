...
SOURCE_FILES=[
 ...
 'runtime/yado_bounded_dnf_relation_policy_inducer_v1.py',
 'runtime/yado_budgeted_stage_policy_v1.py',
 'runtime/yado_conjunctive_rule_inducer_v1.py',
 ...
 'runtime/yado_g2_contextual_stream_capability_adapter_v1.py',
 ...
 'runtime/yado_g2_unified_execution_fabric_v3.py',
 'runtime/yado_g2_canonical_high_scale_binding_runtime_v5.py',
 'runtime/yado_rc8_v36/yado_core_v3_0_rc4_meta_autoevolution.py',
 'runtime/yado_rc8_v36/yado_core_v3_0_rc5_algorithm_genesis.py',
...
task=load(TASK);parent=load(PARENT);fail_parent=load(FAIL_PARENT);head=load(HEAD)
...
def expr_ok(n):
    ...
    if isinstance(n,ast.Constant):
        return isinstance(n.value,int) and not isinstance(n.value,bool) and -10<=n.value<=10
...
def choose_mutation(reference):
    ...
    # V2 taught that structural wrappers can be one-way in the current mutation grammar.
    # V3 admits only a shadow defect for which the same active grammar can reach the parent again.
    for cand in BoundedCompositionalProgramRepairV3._atomic_mutations(
        reference,spec,enable=('binop','compare','boolop','constant')
    ):
        if same_ast(cand,reference):continue
        hs=score(cand,HIDDEN_POINTS,reference)
        if hs>=.85:continue
        reverse=False
        for back in BoundedCompositionalProgramRepairV3._atomic_mutations(
            cand,spec,enable=('binop','compare','boolop','constant')
        ):
            if same_ast(back,reference):
                reverse=True;break
        if reverse:return cand,hs
    return None,None
...
def run_fragment(meta):
    ...
    oracle=SemanticExpressionSynthesizerV1.synthesize(spec,max_ops=4,max_states_per_level=30000)
    ...
    # V3: evidence acquisition is iterative against the current candidate.
    # Every new self-test is a counterexample to the currently believed patch.
    spec_xy={(r['x'],r['y']) for r in spec}
    used=set();tests=[];trace=[];current=mutated
    repair={'source':mutated,'repair_mode':'INITIAL_MUTATED_SOURCE','reason':None}
    for cycle in range(10):
        candidates=[];token_map={}
        for x,y in TEST_GRID:
            if (x,y) in spec_xy or (x,y) in used:continue
            expected=SemanticExpressionSynthesizerV1.predict(oracle,x,y)
            try:got=exec_src(current,x,y)
            except Exception:got='__ERROR__'
            mismatch=1.0 if got!=expected else 0.0
            if mismatch<=0:continue
            token='T-'+sha(meta['token']+'|'+str(cycle)+'|'+str(x)+'|'+str(y))[:14]
            token_map[token]=(x,y,expected)
            candidates.append(EvidenceCandidate(
                token=token,evidence=mismatch,complexity=(abs(x)+abs(y))/10.0,
                risk=0.0,novelty=1.0
            ))
        if not candidates:break
        sel=NeutralEvidenceProfileSelectorV1.select(candidates,complexity_penalty=.003,risk_penalty=.25,novelty_bonus=.02)
        token=sel['selected_token'];x,y,e=token_map[token];used.add((x,y))
        tests.append(((x,y),e))
        before=current
        ...
        trace.append({
          'cycle':cycle,'token':token,'x':x,'y':y,'expected':e,'selected_score':sel['selected_score'],
          'candidate_before_sha256':sha(before),'candidate_after_sha256':sha(current),
          'repair_mode':repair.get('repair_mode'),'repair_reason':repair.get('reason')
        })
    patched=current if current!=mutated else repair.get('source')
    ...
    return {
      ...
      'repair_mode':repair.get('repair_mode'),'repair_reason':repair.get('reason'),
      'reversible_shadow_defect':True,'iterative_counterexample_repair':True,
...
gene={
 'schema':'yado.g2.coding_spec_generalization_real_code_transfer_gene.v3',
 'gene_id':'GENE-G2-CODING-REAL-CODE-TRANSFER-V3-'+digest({'episodes':executed,'parent':parent_gene['gene_digest']})[:16],
 'organ':'THINKING',
 'gene_scope':['THINKING','INTELLIGENCE','CODE','MEMORY','GENERATIVE_EXECUTIVE','LOGIC'],
 'heritage':[parent_gene['gene_id'],parent.get('receipt_sha256'),fail_parent.get('gene_id'),fail_parent.get('receipt_sha256')],
 'mechanism_kind':'REVERSIBLE_REAL_AST_DEFECT_TO_ITERATIVE_SELF_COUNTEREXAMPLE_ORACLE_REPAIR',
 'active_components':[
...
checks={
 ...
 'v2_negative_transfer_consumed':fail_parent.get('status')=='WITHHOLD_G2_CODING_SPEC_GENERALIZATION_REAL_CODE_TRANSFER_V2' and fail_parent.get('repaired_holdout_score')==0.5,
 'v2_one_way_mutation_failure_consumed':fail_parent.get('mutated_holdout_score')>fail_parent.get('repaired_holdout_score'),
 ...
 'all_shadow_defects_reversible':task_count>=6 and all(e.get('reversible_shadow_defect') for e in executed),
 'iterative_counterexample_repair_used':task_count>=6 and all(e.get('iterative_counterexample_repair') for e in executed),
...
status='PASS_SHADOW_G2_CODING_SPEC_GENERALIZATION_REAL_CODE_TRANSFER_V3' if passed else 'WITHHOLD_G2_CODING_SPEC_GENERALIZATION_REAL_CODE_TRANSFER_V3'

experience={
 'schema':'yado.g2.coding_real_code_transfer.experience.v3','status':'TRAINED' if passed else 'WITHHOLD',
...
report={
 'schema':'yado.g2.coding_spec_generalization_real_code_transfer.v3','status':status,'task':task,
 ...
 'next_required_capability':'G2_CODING_WHOLE_FUNCTION_REAL_REPAIR_V1' if passed else 'G2_CODING_SPEC_GENERALIZATION_AND_REAL_CODE_TRANSFER_V4',
