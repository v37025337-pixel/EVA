...
for row in probes[:2]:
    ...
    for attempt in range(2):
        attempt_prompt=prompt if attempt==0 else (
          prompt+'\n\nCONTRACT RETRY: the previous response did not satisfy the transport contract. '
          'Use target_path EXACTLY as '+json.dumps(target_path)+'. Return only the required JSON object and 1-3 exact-string patches.'
        )
        res=call_model(row['url'],row['model_id'],attempt_prompt)
        attempts.append({k:v for k,v in res.items() if k!='content'})
        entry={'provider_key':row['provider_key'],'model_id':row['model_id'],'response':attempts[-1]}
        if not res.get('ok'):
            entry.update({'parsed':False,'evaluation':{'score':0.0,'checks':{}}})
            if res.get('http_status')==429:
                break
            continue
        try:
            ...
            entry.update({'parsed':True,'rationale':str(obj.get('rationale') or '')[:3000],'patch_count':len(patches or []),'new_source':new_src,'evaluation':ev,'attempts':attempts})
            if same_target and all(ev.get('checks',{}).values()):
                break
        except Exception as e:
            entry.update({'parsed':False,'parse_error':type(e).__name__+':'+str(e)[:500],'evaluation':{'score':0.0,'checks':{}},'attempts':attempts})
