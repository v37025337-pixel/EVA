...
TASK=REPO/'architecture/yado-g2-coding-experience-cognitive-consolidation-v2-request.json'
PARENT=REPO/'candidates/kernel-self-generated/g2-experience-conditioned-cognitive-portfolio-v1.json'
FAIL_PARENT=REPO/'candidates/kernel-self-generated/g2-coding-experience-cognitive-consolidation-v1.json'
...
OUT=REPO/'candidates/kernel-self-generated/g2-coding-experience-cognitive-consolidation-v2.json'
EXP=REPO/'experience/yado-coding-cognitive-consolidation-v2.json'
...
task=load(TASK); parent=load(PARENT); fail_parent=load(FAIL_PARENT); v7=load(V7)
...
if fail_parent.get('status')!='WITHHOLD_G2_CODING_EXPERIENCE_COGNITIVE_CONSOLIDATION_V1':
    raise RuntimeError('V1_COGNITIVE_FAILURE_NOT_AVAILABLE')
if fail_parent.get('next_required_capability')!='G2_CODING_EXPERIENCE_COGNITIVE_CONSOLIDATION_V2':
    raise RuntimeError('V1_COGNITIVE_FAILURE_FRONTIER_MISMATCH')
...
for ep in opn.get('episodes') or []:
    ...
    for tr in ep.get('probe_trace') or []:
        ...
        if int(tr.get('hypothesis_count_after') or 0)>1:
            add(logic_rows,'OPEN',tid,{
                'candidate_changed':bool(tr.get('candidate_changed_from_initial')),'result_exact':False,
                'repair_regressed':False,'oracle_exact':False,'real_source':False,
...
for ep in rt3.get('episodes') or []:
    ...
    for tr in ep.get('test_selection_trace') or []:
        ...
        add(logic_rows,'REAL_V3',tid,{
            'candidate_changed':True,'result_exact':False,'repair_regressed':False,
            'oracle_exact':float(ep.get('oracle_hidden_score') or 0)==1.0,
            'real_source':True,'reversible':True,'search_incomplete':True
        },'CONTINUE')
...
def with_unknown_twins(rows):
    ...
    for i,r in enumerate(rows):
        ...
        u=copy.deepcopy(r)
        u['input']={'state_known':False,'unknown_nonce':'U'+str(i)}
        u['expected']='WITHHOLD'
...
# Causal representation ablation: remove the experience-derived routing fields while
# leaving the state known. This must reduce routing accuracy.
intel_abl_cases=[]
for r in intel_blind:
    x=dict(r['input'])
    for k in ('formal_spec_present','hypothesis_set_present','real_source','repair_regressed','reversible'):
        x.pop(k,None)
    intel_abl_cases.append((x,r['expected']))
...
failure_gene_ids=[
    fail_parent.get('logic_gene_id'),fail_parent.get('thinking_gene_id'),
    fail_parent.get('intelligence_gene_id'),fail_parent.get('cognitive_gene_id')
]
heritage=[x for x in parent_gene_ids+recent_gene_ids+failure_gene_ids if x]

logic_gene={
 'schema':'yado.g2.coding_experience_logic_gene.v2',
 'gene_id':'GENE-G2-CODING-EXPERIENCE-LOGIC-V2-'+digest({'program':canonical_program(logic_prog),'heritage':heritage})[:16],
...
thinking_gene={
 'schema':'yado.g2.coding_experience_thinking_gene.v2',
 'gene_id':'GENE-G2-CODING-EXPERIENCE-THINKING-V2-'+digest({'program':canonical_program(thinking_prog),'heritage':heritage})[:16],
...
intel_gene={
 'schema':'yado.g2.coding_experience_intelligence_gene.v2',
 'gene_id':'GENE-G2-CODING-EXPERIENCE-INTELLIGENCE-V2-'+digest({'model':intel_model,'heritage':heritage})[:16],
...
cognitive_gene={
 'schema':'yado.g2.coding_experience_cognitive_layer_gene.v2',
 'gene_id':'GENE-G2-CODING-EXPERIENCE-COGNITIVE-LAYER-V2-'+digest({
 ...
 'mechanism_kind':'TYPED_LOCAL_EXPERIENCE_CONDITIONED_FAIL_CLOSED_LOGIC_THINKING_INTELLIGENCE_COMPOSITE',
 'heritage':heritage,
...
checks={
 'v1_cognitive_failure_consumed':fail_parent.get('status')=='WITHHOLD_G2_CODING_EXPERIENCE_COGNITIVE_CONSOLIDATION_V1',
 'v1_logic_miss_consumed':float(fail_parent.get('logic_fresh') or 0)<.95,
 'v1_intelligence_miss_consumed':float(fail_parent.get('intelligence_fresh') or 0)<.95,
 'typed_local_unknown_representation_repair':True,
 ...
 'intelligence_causal_ablation':intel_fresh-intel_ablation>=.25,
...
status='PASS_SHADOW_G2_CODING_EXPERIENCE_COGNITIVE_CONSOLIDATION_V2' if passed else 'WITHHOLD_G2_CODING_EXPERIENCE_COGNITIVE_CONSOLIDATION_V2'

experience={
 'schema':'yado.g2.coding_experience_cognitive_consolidation.experience.v2',
...
report={
 'schema':'yado.g2.coding_experience_cognitive_consolidation.v2',
