...
LEARNED_EXTERNAL_EVIDENCE_V2 = {'experience_digest': '96b89bd57009c4d0695d33287502a7034c7d249b9bcf74904df6166f5ff7c2b2', 'successful_source_ids': ['OPENAI_EVALS', 'OPENAI_SHELL', 'OPENAI_TOOLS'], 'failed_source_ids': [], 'successful_hosts': ['developers.openai.com'], 'fact_count': 3, 'learning_tokens': ['ability', 'agents', 'building', 'builtin', 'called', 'calling', 'capabilities', 'complete', 'content', 'criteria', 'ensure', 'environment', 'evals', 'evaluations', 'execution', 'extend', 'function', 'generating', 'gives', 'hosted', 'inside', 'local', 'meet', 'model', 'models', 'often', 'outputs', 'programmatic', 'remote', 'responses', 'search', 'servers', 'shell', 'specify', 'style', 'support', 'terminal', 'test', 'that', 'they', 'through', 'tool', 'tools', 'understand', 'using', 'when', 'work', 'writing', 'your']}
import ast
...
def rank_sources(priority: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in SOURCE_CATALOG:
        ...
        score = len(overlap)
        row_host = (urllib.parse.urlsplit(row.get('url', '')).hostname or '').lower()
        score += 1 if row_host in LEARNED_EXTERNAL_EVIDENCE_V2.get('successful_hosts', []) else 0
        score -= 1 if row.get('id') in LEARNED_EXTERNAL_EVIDENCE_V2.get('failed_source_ids', []) else 0
