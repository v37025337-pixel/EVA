...
LEARNED_EXTERNAL_EVIDENCE_V2 = {'experience_digest': '0ad2b9b7a9302fd6a6bf87c1ef2fb5aef8a1faaaee137306e7d61e2e041e6cb1', 'successful_source_ids': ['PYTHON_AST', 'PYTHON_HTMLPARSER'], 'successful_hosts': ['docs.python.org'], 'fact_count': 48, 'learning_tokens': ['3147', 'abstract', 'addition', 'alternatives', 'applications', 'arguments', 'asterisk', 'attribute', 'attributes', 'between', 'built', 'call', 'capturing', 'change', 'child', 'class', 'classes', 'code', 'compare', 'compiled', 'concrete', 'constructor', 'created', 'current', 'defined', 'distinguish', 'documentation', 'each', 'ever', 'example', 'extra', 'find', 'from', 'gives', 'grammar', 'hand', 'handles', 'have', 'helps', 'identifier', 'inherit', 'instance', 'instances', 'into', 'itself', 'keys', 'keyword', 'kwargs', 'left', 'like', 'lists', 'looks', 'mapping', 'marked', 'matchmapping', 'might', 'module', 'more', 'names', 'need', 'node', 'nodes', 'null', 'object', 'only', 'optional', 'parameter', 'possible', 'process', 'production', 'programmatically', 'python', 'release', 'represented', 'rest', 'right', 'rules', 'sequences', 'side', 'specific', 'sums', 'supplied', 'symbol', 'syntax', 'there', 'these', 'this', 'tree', 'trees', 'type', 'using', 'values', 'what', 'which', 'with', 'zero']}
import ast
import hashlib
import html
import ipaddress
...
MAX_FACTS = 24
SOURCE_CATALOG = [{'id': 'PYTHON_AST', 'url': 'https://docs.python.org/3/library/ast.html', 'tags': ['python', 'ast', 'source', 'rewrite', 'compile', 'code', 'syntax']}, {'id': 'PYTHON_URLOPEN', 'url': 'https://docs.python.org/3/library/urllib.request.html', 'tags': ['network', 'http', 'https', 'request', 'url', 'internet', 'read-only']}, {'id': 'PYTHON_HTMLPARSER', 'url': 'https://docs.python.org/3/library/html.parser.html', 'tags': ['html', 'parser', 'document', 'web', 'learning', 'external']}, {'id': 'MDN_HTTP', 'url': 'https://developer.mozilla.org/en-US/docs/Web/HTTP/Overview', 'tags': ['http', 'https', 'web', 'network', 'protocol', 'internet']}, {'id': 'W3C_WEBARCH', 'url': 'https://www.w3.org/TR/webarch/', 'tags': ['web', 'architecture', 'uri', 'resource', 'protocol', 'internet']}, {'id': 'PYTHON_UNITTEST', 'url': 'https://docs.python.org/3/library/unittest.html', 'tags': ['test', 'regression', 'verification', 'rollback', 'software', 'quality']}]
ALLOWED_HOSTS = {urllib.parse.urlsplit(row['url']).hostname.lower() for row in SOURCE_CATALOG}
FORBIDDEN_HEADER_NAMES = {'authorization', 'proxy-authorization', 'cookie', 'set-cookie', 'x-api-key', 'api-key'}
ALLOWED_CONTENT_TYPES = {'text/html', 'text/plain', 'application/json', 'application/problem+json', 'application/dns-json'}
...
def load_current_priority() -> dict[str, Any]:
    ...
    if receipt.exists():
        try:
            ...
            pass
    return {'code': 'AUTONOMOUS_LEARNING_BOOTSTRAP', 'area': 'EXTERNAL_EVIDENCE_AND_SELF_EVOLUTION', 'recommended_action': 'study public technical sources and bind verified experience to future code evolution'}

def tokens(priority: dict[str, Any]) -> set[str]:
    raw = ' '.join((str(priority.get(k, '')) for k in ('code', 'area', 'recommended_action')))
    out = set(re.findall('[a-zA-Z][a-zA-Z0-9_-]{2,}', raw.lower()))
    out |= {'python', 'code', 'web', 'network', 'test', 'source', 'learning'}
    return out

def rank_sources(priority: dict[str, Any]) -> list[dict[str, Any]]:
    q = tokens(priority)
    q |= set(LEARNED_EXTERNAL_EVIDENCE_V2.get('learning_tokens', []))
    ranked = []
    for row in SOURCE_CATALOG:
        tags = set(row['tags'])
        overlap = sorted(q & tags)
        score = len(overlap)
        score += 2 if row.get('id') in LEARNED_EXTERNAL_EVIDENCE_V2.get('successful_source_ids', []) else 0
...
def synthesize_recall_module(experience: dict[str, Any], path: Path) -> dict[str, Any]:
    ...
    payload = {'schema': SCHEMA, 'experience_digest': experience['experience_digest'], 'priority': experience['priority'], 'facts': facts}
    source = 'from __future__ import annotations\n\nLEARNED_EXPERIENCE = ' + repr(payload) + "\n\ndef recall(query: str, limit: int = 8):\n    q = {x for x in str(query).lower().split() if x}\n    ranked = []\n    for row in LEARNED_EXPERIENCE['facts']:\n        text = row['text']\n        score = sum(1 for x in q if x in text.lower())\n        if score:\n            ranked.append((score, row))\n    ranked.sort(key=lambda x: (-x[0], x[1]['source_id'], x[1]['text']))\n    return [row for _, row in ranked[:max(1, min(int(limit), 32))]]\n\ndef component():\n    return {'schema': 'yado.learned_recall_capability.v1', 'experience_digest': LEARNED_EXPERIENCE['experience_digest'], 'fact_count': len(LEARNED_EXPERIENCE['facts']), 'external_code_executed': False, 'canonical_active': False}\n"
...
def main() -> None:
    ...
    for row in selected:
        try:
            ...
            source_results.append({'source_id': row['id'], 'url': row['url'], 'selection_score': row['score'], 'selection_overlap': row['overlap'], 'facts': facts, 'fact_count': len(facts), 'network': r})
