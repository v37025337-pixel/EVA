...
FRESH_TRANSFER_SOURCES = (
    ("python-algorithms", "https://github.com/ishtiaqmahmood/python-algorithms.git"),
    ("sympy", "https://github.com/sympy/sympy.git"),
)


def discover_transfer_tasks(root: Path, sources: dict[str, dict[str, str]]) -> list[dict[str, Any]]:
    """Apply the existing six-task floor to the combined external corpus.

    Collection uses source structure, never candidate success. Preserve the
    original sparse repository and deduplicate identical target expressions.
    Every result carries the actual repository, revision and relative path.
    """
    rows = discover_extended_tasks(root, set(), limit=32)
    tasks, seen = [], set()
    for row in rows:
        repository_name, relative = row['path'].split('/', 1)
        if repository_name not in sources:
            raise ValueError('UNREGISTERED_TRANSFER_SOURCE:' + repository_name)
        marker = row['target_expression_sha256']
        if marker in seen:
            continue
        seen.add(marker)
        tasks.append({**row, 'path': relative,
                      'source_repository': sources[repository_name]['repository'],
                      'source_head_sha': sources[repository_name]['head_sha']})
    if len(tasks) < 6:
        raise RuntimeError(f'INSUFFICIENT_DISTINCT_TRANSFER_TASKS:{len(tasks)}<6')
    return tasks
    ...
    *,
    ...
    if candidate:
        ...
        try:
            ...
            failure_reason = f"{type(exc).__name__}:{exc}"
    passed = bool(candidate and len(checks) == len(hidden) and checks and all(checks))
    return {
...
def run(manifest: Path, output: Path) -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="yado-second-cycle-improvement-") as temp:
            ...
            fresh_root = root / 'third-transfer-external'
            fresh_root.mkdir()
            fresh_sources = {}
            for name, repository in FRESH_TRANSFER_SOURCES:
                fresh_repo = clone_latest(fresh_root, repository, name)
                fresh_sources[name] = {'repository': repository,
                                       'head_sha': _git(fresh_repo, 'rev-parse', 'HEAD')}
            fresh_tasks = discover_transfer_tasks(fresh_root, fresh_sources)
            fresh_rows = [
                synthesis_probe(
                    evolved,
                    task,
                    task['source_head_sha'],
    ...
    report = {
        ...
        "fresh_transfer": {
            ...
            "duplicate_target_expressions_removed": True,
            "collection_basis": "SOURCE_STRUCTURE_WITHOUT_CANDIDATE_OUTCOMES",
        },
    ...
    print(json.dumps({
        ...
        "fresh_transfer_heads": {name: row['head_sha'] for name, row in fresh_sources.items()},
