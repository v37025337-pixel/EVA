...
def _bitwise_templates(keys: list[str]) -> list[dict[str, str]]:
    """Try the original grammar before its generic relational extension.

    Several expressions can fit the same finite training set.  Extending the
    search must not replace an existing fit with a shorter, newly added alias.
    Keep the original bit predicates and ordering as the first search stage;
    relational closure is a fallback when that stage cannot fit the training.
    """
    q = {key: f"inputs[{key!r}]" for key in keys}
    bases = _integer_bitwise_bases(keys)
    original = [base for base in bases if not base["template"].startswith("SHIFT_CLEAR_LOW:")]
    for base in bases:
        if base["template"].startswith("EXTRACT_BIT:"):
            prefix = base["template"].split(":", 1)[1]
            for name, value in (("ONE", 1), ("ZERO", 0)):
                original.append({
                    "template": f"BIT_IS_{name}:{prefix}",
                    "expression": f"({base['expression']} == {value})",
                })
    order = lambda row: (len(row["expression"]), row["template"], row["expression"])
    original.sort(key=order)
    out = list(bases)
    ...
    original_expressions = {row["expression"] for row in original}
    extension = {row["expression"]: row for row in out
                 if row["expression"] not in original_expressions}
    return original + sorted(extension.values(), key=order)
